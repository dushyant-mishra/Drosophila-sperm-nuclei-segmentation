...
print(f"\n--- NEW SESSION STARTED (v5.4 hybrid): {os.path.basename(__file__)} ---")
...
CONFIG = {
    ...
    "HYBRID_REPAIR_MAX_COST":       3.6,       # v5.4 fragment-repair gate; lower is stricter
...
def export_quality_overlays(out_dir, slice_cache, df_tracked, track_summary):
    ...
    for _, row in track_summary.iterrows():
        ...
        if "is_biological_candidate" in track_summary.columns:
...
def track_across_slices_global_assignment(detections_df, cfg):
    """
    V5.4 hybrid tracker: link each slice with a global assignment cost matrix.
...
def track_across_slices_hybrid_repair(detections_df, cfg):
    """
    V5.4 hybrid tracker: keep the conservative legacy tracker, then run a
...
def flag_quality_tracks(df_tracks, cfg):
    ...
    Adds strict and softer biological-candidate audit columns:
        is_quality_track (bool): strict legacy audit; any flag fails
        quality_flags (str):     comma-separated strict/warning flag names
        is_biological_candidate: passes hard structural flags; warnings allowed
        hard_flags:              long/tortuous/extreme PSF-sensitive failures
    ...
    def build_flags(mask_list):
        n_rows = len(df_tracks)
        strings = [""] * n_rows
    ...
    n = len(df_tracks)
    flag_strs, any_flagged = build_flags(strict_masks)
    hard_strs, any_hard = build_flags(hard_masks)
    warning_strs, any_warning = build_flags(warning_masks)
...
PARAM_DESCRIPTIONS = {
    ...
    "TRACKING_BACKEND": "Which tracking engine to use. v5.4 default is hybrid_repair: legacy tracking first, followed by a conservative fragment-repair pass. Other options are legacy and global_assignment.",
    ...
    "HYBRID_REPAIR_MAX_COST": "V5.4 hybrid repair only. Candidate fragment merges above this cost are rejected. Lower values are safer and leave more fragments split.",
    "HYBRID_REPAIR_MAX_GAP_SLICES": "V5.4 hybrid repair only. Maximum blank slices allowed between two fragments being considered for repair.",
    "HYBRID_REPAIR_MAX_FRAGMENT_SLICES": "V5.4 hybrid repair only. At least one side of a repaired link must have this many slices or fewer, so the pass targets fragments rather than rewriting stable tracks.",
    "HYBRID_REPAIR_MAX_LINK_DIST_UM": "V5.4 hybrid repair only. Hard distance gate for fragment merges unless there is direct bounding-box overlap evidence.",
    "HYBRID_REPAIR_MIN_OVERLAP": "V5.4 hybrid repair only. Preferred minimum bounding-box overlap for a repair link. Very close non-overlap links can still pass if the total cost is low.",
    "HYBRID_REPAIR_MAX_FINAL_LENGTH_UM": "V5.4 hybrid repair only. Rejects a proposed merge if the estimated merged 3D length would exceed this value.",
...
class SpermGUI:
    ...
    def _launch_parameter_tuner(self, mode):
        """Launch the external V5.4 hybrid tuner without blocking the GUI."""
        ...
        tuner_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "utils", "tune_parameters_Saturnv5_4.py")
        ...
        try:
            ...
            if roi_mask is not None:
                roi_path = os.path.join(outdir, "last_drawn_roi_saturnv5_4_tune.tif")
