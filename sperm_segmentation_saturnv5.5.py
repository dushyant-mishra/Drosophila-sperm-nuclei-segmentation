...
print(f"\n--- NEW SESSION STARTED (v5.5 hybrid): {os.path.basename(__file__)} ---")
...
def track_across_slices_global_assignment(detections_df, cfg):
    """
    V5.5 hybrid tracker: link each slice with a global assignment cost matrix.
...
def track_across_slices_hybrid_repair(detections_df, cfg):
    """
    V5.5 hybrid tracker: keep the conservative legacy tracker, then run a
...
PARAM_DESCRIPTIONS = {
    ...
    "HYBRID_REPAIR_MAX_COST": "V5.5 hybrid repair only. Candidate fragment merges above this cost are rejected. Lower values are safer and leave more fragments split.",
    "HYBRID_REPAIR_MAX_GAP_SLICES": "V5.5 hybrid repair only. Maximum blank slices allowed between two fragments being considered for repair.",
    "HYBRID_REPAIR_MAX_FRAGMENT_SLICES": "V5.5 hybrid repair only. At least one side of a repaired link must have this many slices or fewer, so the pass targets fragments rather than rewriting stable tracks.",
    "HYBRID_REPAIR_MAX_LINK_DIST_UM": "V5.5 hybrid repair only. Hard distance gate for fragment merges unless there is direct bounding-box overlap evidence.",
    "HYBRID_REPAIR_MIN_OVERLAP": "V5.5 hybrid repair only. Preferred minimum bounding-box overlap for a repair link. Very close non-overlap links can still pass if the total cost is low.",
    "HYBRID_REPAIR_MAX_FINAL_LENGTH_UM": "V5.5 hybrid repair only. Rejects a proposed merge if the estimated merged 3D length would exceed this value.",
...
class SpermGUI:
    ...
    def _launch_parameter_tuner(self, mode):
        """Launch the external V5.5 hybrid tuner without blocking the GUI."""
        ...
        tuner_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "utils", "tune_parameters_Saturnv5_5.py")
        ...
        try:
            ...
            if roi_mask is not None:
                roi_path = os.path.join(outdir, "last_drawn_roi_saturnv5_5_tune.tif")
