...
CONFIG = {
    ...
    # ── image enhancement ─────────────────────────────────────────────────────
    "CLAHE_CLIP":   0.1055,  # 3D Optimized
    "CLAHE_KERNEL": 64,
    "BG_SIGMA":     5.296,   # 3D Optimized
    "RIDGE_SIGMAS": [1, 2, 3, 4],

    # ── hysteresis threshold ──────────────────────────────────────────────────
    "THRESHOLD_HI": 75.86,   # 3D Optimized
    "THRESHOLD_LO": 60.97,   # 3D Optimized
    ...
    "MIN_SKEL_LEN_PX":        5.32, # 3D Optimized
    "MAX_GEODESIC_LEN_PX":    65.0, # backend actively cuts chains longer than this
    "MAX_WIDTH_PX":           8.81, # 3D Optimized
    "MIN_LENGTH_WIDTH_RATIO": 1.52, # 3D Optimized

    # ── NEW topology filters ──────────────────────────────────────────────────
    "MAX_BRANCH_NODES": 9,        # 3D Optimized
    "MAX_TORTUOSITY": 6.16,       # 3D Optimized
    "MAX_ENDPOINT_COUNT": 3,      # 3D Optimized
...
class SpermGUI:
    ...
    def __init__(self, root):
        ...
        self.sidebar_container = tk.Frame(root, width=280, bg='#f0f0f0')
        ...
        def _on_sidebar_configure(event):
            # Update scrollregion to encompass the entire inner frame
            # Force update to ensure geometry is calculated
            self.sidebar.update_idletasks()
            rw = self.sidebar.winfo_reqwidth()
            rh = self.sidebar.winfo_reqheight()
            self.sidebar_canvas.configure(scrollregion=(0, 0, rw, rh))
            print(f"DEBUG: Sidebar scrollregion updated to {rw}x{rh}")
    ...
    def run_ai_analysis(self):
        ...
        csv_summary_str = ts.head(100).to_csv(index=False)
