#!/usr/bin/env python3
"""
Saturn v5.7.1 dual-head U-Net-primary parameter tuner.

This tuner evaluates preprocessing profiles, classical segmentation parameters,
and tracking parameters using the v5.7.1 pipeline interface. It always passes ROI,
exclusion mask, and a stack preprocessing context to segmentation calls.

Model C epoch 3 and its foreground/core thresholds are frozen production
inputs. In comparative mode, tracking morphology is descriptive and is not an
optimization term or a link veto.
"""
...
module_name = "sperm_segmentation_saturnv5_7_1"
module_path = os.path.join(parent_dir, "sperm_segmentation_saturnv5.7.1.py")
...
TRACKING_CONFIG_KEYS = (
    ...
    "COMPARATIVE_TRACKING_MORPHOLOGY_NEUTRAL",
    "ASSIGNMENT_MAX_COST",
...
def build_unet_probability_cache(cfg, cache_dir, force=False):
    ...
    cache = {}
    core_cache = {}
    dual_head = str(cfg.get("UNET_OUTPUT_MODE", "single_head")).lower() == "dual_head"
    missing = []
    for z in z_values_eval:
        ...
        if (
            ...
            if arr.shape != np.asarray(images_to_eval[0]).shape:
                ...
                if dual_head:
                    core = tifffile.imread(str(core_path)).astype(np.float32)
                    if core.shape != arr.shape:
                        missing.append(int(z))
                        cache.pop(int(z), None)
                    else:
                        core_cache[int(z)] = np.clip(core, 0.0, 1.0)
        else:
            missing.append(int(z))
    ...
    if missing:
        ...
        for z in missing:
            ...
            prob = np.asarray(heads["foreground"], dtype=np.float32)
            ...
            if dual_head:
                if core is None:
                    raise RuntimeError(
                        "Dual-head tuner cache requires a core probability map"
                    )
                core = np.clip(np.asarray(core, dtype=np.float32), 0.0, 1.0)
                tifffile.imwrite(
                    str(cache_dir / f"z{int(z):02d}_unet_core_probability.tif"),
                    core,
                )
...
def _tracking_row_key(row):
    value = row.get("source_instance_key")
    if value is not None and str(value) not in {"", "nan", "None"}:
        return str(value)
    return f"z{int(row['z_slice'])}:s{int(row.get('sperm_id', row.name))}"


def _bbox_overlaps_rows(first, second):
    required = (
        "bbox_min_y", "bbox_min_x", "bbox_max_y", "bbox_max_x"
    )
    if any(column not in first or column not in second for column in required):
        return False
    values = [float(first[column]) for column in required] + [
        float(second[column]) for column in required
    ]
    if not all(np.isfinite(value) for value in values):
        return False
    return (
        min(float(first["bbox_max_y"]), float(second["bbox_max_y"]))
        > max(float(first["bbox_min_y"]), float(second["bbox_min_y"]))
        and min(float(first["bbox_max_x"]), float(second["bbox_max_x"]))
        > max(float(first["bbox_min_x"]), float(second["bbox_min_x"]))
    )


def reciprocal_tracking_integrity(detections, tracked, cfg):
    """Measure adjacent-plane continuity without using nucleus morphology."""
    if detections.empty or tracked.empty:
        return {
            "reciprocal_candidate_count": 0,
            "reciprocal_candidate_recovery_fraction": 0.0,
            "accepted_nonreciprocal_fraction": 1.0,
        }

    max_distance_um = float(
        cfg.get("CONSERVATIVE_MAX_CENTROID_JUMP_UM", 5.2397)
    )
    um_per_px = float(cfg.get("UM_PER_PX_XY", 1.0))
    by_z = {
        int(z): group.reset_index(drop=True)
        for z, group in detections.groupby("z_slice")
    }
    reciprocal_pairs = set()
    z_values = sorted(by_z)
    for first_z, second_z in zip(z_values, z_values[1:]):
        if second_z - first_z != 1:
            continue
        first = by_z[first_z]
        second = by_z[second_z]
        first_xy = first[["centroid_x", "centroid_y"]].to_numpy(dtype=float)
        second_xy = second[["centroid_x", "centroid_y"]].to_numpy(dtype=float)
        if not len(first_xy) or not len(second_xy):
            continue
        first_tree = cKDTree(first_xy)
        second_tree = cKDTree(second_xy)
        first_to_second_dist, first_to_second = second_tree.query(first_xy)
        _, second_to_first = first_tree.query(second_xy)
        for first_index, second_index in enumerate(first_to_second):
            second_index = int(second_index)
            if int(second_to_first[second_index]) != first_index:
                continue
            if float(first_to_second_dist[first_index]) * um_per_px > max_distance_um:
                continue
            first_row = first.iloc[first_index]
            second_row = second.iloc[second_index]
            if not _bbox_overlaps_rows(first_row, second_row):
                continue
            reciprocal_pairs.add(
                (_tracking_row_key(first_row), _tracking_row_key(second_row))
            )

    accepted_pairs = set()
    for _, group in tracked.sort_values(["track_id", "z_slice"]).groupby("track_id"):
        records = [row for _, row in group.iterrows()]
        for first, second in zip(records, records[1:]):
            if int(second["z_slice"]) - int(first["z_slice"]) == 1:
                accepted_pairs.add(
                    (_tracking_row_key(first), _tracking_row_key(second))
                )

    recovered = reciprocal_pairs & accepted_pairs
    nonreciprocal = accepted_pairs - reciprocal_pairs
    return {
        "reciprocal_candidate_count": int(len(reciprocal_pairs)),
        "reciprocal_candidate_recovery_fraction": float(
            len(recovered) / max(len(reciprocal_pairs), 1)
        ),
        "accepted_nonreciprocal_fraction": float(
            len(nonreciprocal) / max(len(accepted_pairs), 1)
        ),
    }
...
def summarize_tracking_candidate(detections, tracked, tracks, cfg):
    ...
    continuity = reciprocal_tracking_integrity(
        detections,
        tracked,
        cfg,
    )
    ...
    score = (
        ...
        + (1.0 - continuity["reciprocal_candidate_recovery_fraction"]) * 120.0
        + continuity["accepted_nonreciprocal_fraction"] * 80.0
    )
    result = {
        "score": float(score),
        "n_2d": n_detections,
...
def run_profile_mode(outdir, base_cfg):
    ...
    best = records[0]
    n = next_run_number(outdir, "profile_comparison_v5_7_1_*.csv")
    pd.DataFrame(records).to_csv(outdir / f"profile_comparison_v5_7_1_{n:03d}.csv", index=False)
    with open(outdir / f"profile_comparison_v5_7_1_{n:03d}.json", "w", encoding="utf-8") as f:
    ...
    with open(outdir / f"best_preprocessing_profile_v5_7_1_{n:03d}.json", "w", encoding="utf-8") as f:
        json.dump(preset, f, indent=2)
    with PdfPages(outdir / f"profile_review_v5_7_1_{n:03d}.pdf") as pdf:
        ...
        fig.suptitle("Saturn v5.7.1 preprocessing profile review")
...
def generate_candidate_review_pdf(
    ...
    n = next_run_number(outdir, f"candidate_visual_review_v5_7_1_{mode}_*.pdf")
    path = outdir / f"candidate_visual_review_v5_7_1_{mode}_{n:03d}.pdf"
    with PdfPages(path) as pdf:
        for slice_idx, (img, z_idx) in enumerate(
            ...
            for col, (record, segs) in enumerate(rendered):
                ...
                axes[0, col].axis("off")
            fig.suptitle(f"Saturn v5.7.1 {mode} candidate review, Z={z_idx}")
            pdf.savefig(fig, dpi=180)
            plt.close(fig)
    if mode in {"unet_rescue", "unet_primary"} and selected:
        ...
        support_n = next_run_number(
            outdir,
            "selected_unet_probability_review_v5_7_1_*.pdf",
        )
        support_path = (
            outdir
            / f"selected_unet_probability_review_v5_7_1_{support_n:03d}.pdf"
        )
...
def aggregate_stratum_results(
    ...
    n = next_run_number(outdir, f"shared_{mode}_params_v5_7_1_*.json")
    preset_path = outdir / f"shared_{mode}_params_v5_7_1_{n:03d}.json"
    with open(preset_path, "w", encoding="utf-8") as f:
        json.dump(preset, f, indent=2)
    pd.DataFrame(summaries).to_csv(
        outdir / f"shared_{mode}_candidate_comparison_v5_7_1_{n:03d}.csv",
        index=False,
    )
    with open(
        outdir / f"shared_{mode}_candidate_comparison_v5_7_1_{n:03d}.json",
        "w",
        encoding="utf-8",
    ) as f:
...
def save_results(outdir, mode, best, records, cfg, review_candidates=6):
    n = next_run_number(outdir, f"best_{mode}_params_v5_7_1_*.json")
    ...
    with open(outdir / f"best_{mode}_params_v5_7_1_{n:03d}.json", "w", encoding="utf-8") as f:
        json.dump(preset, f, indent=2)
    with open(outdir / f"tuning_results_saturnv5_7_1_{mode}.json", "w", encoding="utf-8") as f:
        json.dump(records, f, indent=2)
    pd.DataFrame(records).to_csv(outdir / f"tuning_results_saturnv5_7_1_{mode}.csv", index=False)
    review_count = min(max(1, int(review_candidates)), len(records))
    with open(
        outdir / f"candidate_review_queue_v5_7_1_{mode}_{n:03d}.json",
        "w",
        encoding="utf-8",
    ) as f:
        json.dump(records[:review_count], f, indent=2)
    with open(outdir / f"tuning_summary_saturnv5_7_1_{mode}.txt", "w", encoding="utf-8") as f:
...
def run_self_check():
    checks = []
    checks.append(("v5.7.1 module import", hasattr(segmentation, "segment_slice")))
    ...
    checks.append(("v5.7.1 version string", getattr(segmentation, "_VERSION", "") == VERSION))
    ...
    checks.append(("profile output naming", "profile_comparison_v5_7_1_001.csv".startswith("profile_comparison_v5_7_1_")))
    checks.append(("segmentation output naming", "best_segmentation_params_v5_7_1_001.json".startswith("best_segmentation_params_v5_7_1_")))
    ...
    print("Saturn v5.7.1 tuner self-check passed")


def main(argv=None):
    parser = argparse.ArgumentParser(description="Saturn v5.7.1 dual-head parameter tuner")
    parser.add_argument(
        "--mode",
        choices=[
            "profile",
    ...
    if args.mode in unet_modes:
        ...
        cfg["UNET_FAIL_HARD"] = True
        if str(cfg.get("UNET_OUTPUT_MODE", "single_head")).lower() == "dual_head":
            if args.mode == "unet_primary":
                raise SystemExit(
                    "Dual-head Model C segmentation is frozen at foreground "
                    "0.60 and core 0.50. Use --mode unet_primary_tracking to "
                    "validate tracking without reopening checkpoint selection."
                )
            if not math.isclose(float(cfg.get("UNET_FOREGROUND_THRESHOLD", -1)), 0.60):
                raise SystemExit("Dual-head foreground threshold must remain 0.60")
            if not math.isclose(float(cfg.get("UNET_CORE_THRESHOLD", -1)), 0.50):
                raise SystemExit("Dual-head core threshold must remain 0.50")
