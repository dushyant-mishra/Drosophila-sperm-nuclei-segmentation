...
CONFIG = {
    ...
    "AUDIT_MIN_SLICES":        2,       # Flag tracks with fewer slices than this
    ...
    try:
        with pd.ExcelWriter(excel_path, engine='xlsxwriter') as writer:
            ...
            dictionary_data = [
                ...
                ["3D Volume (µm³)", "Σ(Area_slice × Z_step)", "Slice-wise Riemann-sum approximation of nuclear volume."],
                ["Effective Thickness (µm)", "2 * sqrt((V_3D / L_3D) / π)", "Cylinder-equivalent average diameter; useful for relative comparisons."],
                ["Pitch Angle (degrees)", "|arcsin(ΔZ / Euclidean_3D)| × 180/π", "Absolute plunge angle relative to the imaging plane."],
                ["Taper Ratio", "max(cross-sectional area) / min(cross-sectional area)", "Large values suggest abrupt narrowing, fusion, or segmentation instability."],
    ...
    try:
        with PdfPages(pdf_path) as pdf:
            ...
            if df_tracks is not None and not df_tracks.empty:
                ...
                audit_counts = get_audit_flag_counts(df_tracks, cfg)
            ...
            guide_full = (
                ...
                "   Meaning: Slice-wise Riemann-sum estimate of nuclear volume.\n\n"
                "6. Effective Thickness (Average Diameter, µm)\n"
                "   Formula: D_avg = 2 × sqrt((V_3D / L_3D) / π)\n"
                "   Meaning: Cylinder-equivalent average diameter; best used comparatively.\n\n"
                ...
                "   Meaning: Large values indicate abrupt narrowing, fusion, or segmentation instability.\n\n"
                ...
                "   Interpretation: Audit flags suspicious tracks after tracking. It should annotate the population, while reports distinguish all tracks from audit-passed tracks.\n"
            )
...
def generate_pptx_report(out_dir, df, df_summary, um, df_tracks=None):
    ...
    try:
        ...
        if df_tracks is not None and not df_tracks.empty:
            ...
            audit_counts = get_audit_flag_counts(df_tracks, CONFIG)
            n_single = audit_counts["single_slice"]
            n_outlier_other = audit_counts["shape_outliers"]
            n_single_only = min(n_single, n_flagged)
...
PARAM_TITLES = {
    ...
    "AUDIT_MAX_THICKNESS_UM": "Audit: maximum effective thickness (µm)",
    "AUDIT_MAX_TAPER_RATIO": "Audit: maximum taper ratio",
    "AUDIT_MIN_SLICES": "Audit: minimum slices required"
}
...
PARAM_DESCRIPTIONS = {
    ...
    "AUDIT_MAX_THICKNESS_UM": "Audit only. Flags tracks that look too thick for a nucleus. Useful for fused or swollen objects.",
    "AUDIT_MAX_TAPER_RATIO": "Audit only. Flags tracks with extreme change from thickest to thinnest slice. Large values often indicate unstable segmentation or merged objects.",
    "AUDIT_MIN_SLICES": "Audit only. Minimum number of slices required to avoid the single-slice flag. Set to 1 if single-slice nuclei should remain in the audit-passed population."
}
