#!/usr/bin/env python3
"""
Evolutionary Parameter Tuner for Saturn V5.3 Prototype Tracking

Biology- and hardware-aware tuner for the *tracking* stage of the
Drosophila sperm nucleus pipeline.

This version is aligned to the V5.3 prototype assumptions:
...
- imports from sperm_segmentation_saturnv5.3.py
...
-----
GUI mode:
    python utils/tune_parameters_Saturnv5_3.py

CLI mode:
    python utils/tune_parameters_Saturnv5_3.py --dir "path/to/images" --slices 0-12
...
# -----------------------------------------------------------------------------
# Import V5.3 prototype pipeline functions directly for fast in-process execution
# -----------------------------------------------------------------------------
# Add parent directory to path
parent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.append(parent_dir)

# Since the filename has a dot (v5.3.py), we use importlib to load it properly
import importlib.util
module_name = "sperm_segmentation_saturnv5_3" # alias for internal use
module_path = os.path.join(parent_dir, "sperm_segmentation_saturnv5.3.py")
...
ROI_SAVE_PATH = DEFAULT_OUTPUT_DIR / "last_drawn_roi_saturnv5_3_tune.tif"
...
PARAM_SPACE = [
    ...
    ("ASSIGNMENT_MAX_COST",                3.0,   7.2,   False),
...
def is_safe_tracking_candidate(record):
    ...
    try:
        ...
        taper_frac = record.get("taper_frac", 0)
        ...
        return (
            float(zspan) >= 0.5
            and float(single_frac) <= 0.52
            and float(l3d_med) <= 11.6
            and float(l3d_mean) <= 12.9
            and float(taper_frac) <= 0.40
        )
    except Exception:
        return False
...
def score_run(df_2d, df_tracks, z_step_um):
    ...
    # 4. Derived quantities are PSF-sensitive, but taper inflation is a strong
    # warning sign for over-linking in the v5.3 global-assignment tracker.
    score -= 0.45 * n_thick
    score -= 1.05 * n_taper
    ...
    if taper_frac > 0.34:
        score -= 500.0 * (taper_frac - 0.34)
    if thick_frac > 0.62:
        score -= 220.0 * (thick_frac - 0.62)

    # 8. Penalize fragmentation signatures exposed by the V5.3 tracker.
...
def objective_fn(x, um_per_px, z_step_um):
    ...
    cfg["TRACKING_BACKEND"] = "global_assignment"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evolutionary parameter tuner for Saturn V5.3 prototype segmentation/tracking"
    )
    ...
    parser.add_argument("--um-per-px", type=float, default=None,
                       help="Override calibration (um/px). Defaults to V5.3 CONFIG.")
    parser.add_argument("--z-step-um", type=float, default=None,
                       help="Override z-step in um. Defaults to V5.3 CONFIG.")
    ...
    ROI_SAVE_PATH = outdir / "last_drawn_roi_saturnv5_3_tune.tif"
    ...
    x0 = []
    print("\nSeed parameters (from V5.3 CONFIG):")
    ...
    print(f"\n{'='*78}")
    print(f"  EVOLUTIONARY {mode_label} PARAMETER TUNING (V5.3 PROTOTYPE)")
    ...
    if args.mode == "tracking":
        best_params["TRACKING_BACKEND"] = "global_assignment"
    ...
    existing = sorted(outdir.glob(f"best_{args.mode}_params_v5_3_*.json"))
    next_num = 1
    if existing:
        for ep in existing:
            try:
    ...
    best_param_filename = f"best_{args.mode}_params_v5_3_{next_num:03d}.json"
    ...
    if args.mode == "tracking":
        safe_candidates_path = outdir / f"safe_tracking_candidates_v5_3_{next_num:03d}.json"
        ...
        if safe_tracking_best:
            ...
            safe_params["TRACKING_BACKEND"] = "global_assignment"
            safe_param_filename = f"best_safe_tracking_params_v5_3_{next_num:03d}.json"
    ...
    history_path = outdir / f"tuning_results_saturnv5_3_{args.mode}.json"
    ...
    summary_path = outdir / f"tuning_summary_saturnv5_3_{args.mode}.txt"
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write(f"SATURN V5.3 PROTOTYPE {mode_label} TUNING SUMMARY\n")
        f.write("=" * 72 + "\n")
        ...
        if args.mode == "tracking":
            ...
            f.write("  safe candidate rule: zspan_median_um >= 0.5, single_frac <= 0.52, l3d_median_um <= 11.6, l3d_mean_um <= 12.9, taper_frac <= 0.40\n")
        ...
        if args.mode == "segmentation":
            ...
            f.write("  This V5.3 prototype tuner preserves biologically plausible single-slice nuclei for this Leica\n")
    ...
    if args.mode == "tracking" and safe_param_filename:
        ...
        print(f"  CLI: python sperm_segmentation_saturnv5.3.py --batch --params \"{safe_param_path}\"")
    else:
        print(f"  GUI: Click 'Load Tuned Params' -> select {best_param_filename}")
        print(f"  CLI: python sperm_segmentation_saturnv5.3.py --batch --params \"{best_param_path}\"")
