...
try:
    from sperm_segmentation_saturn import (
        CONFIG,
        segment_slice,
        measure_spermatids,
        normalize_display,
    ...
    print("Error: Could not import 'sperm_segmentation_v11_combined.py'. Make sure it's in the same directory.")
    sys.exit(1)

ROI_SAVE_PATH = "last_drawn_roi_saturn.tif"
    ...
    try:
        for img in images_to_eval:
            ...
            if count > 0:
                ...
                median_um = median_length_px * um_per_px
...
def main():
    ...
    parser.add_argument("--um-per-px", type=float, default=None, help="Calibration (um/px). Defaults to CONFIG value if not specified.")
    parser.add_argument("--new-roi", action="store_true", help="Force drawing a new ROI, overriding saved ones.")
    args = parser.parse_args()
    
    # Use command-line override or the CONFIG default
    ...
    if roi_mask is None:
        ...
        selector = PolygonSelector(ax, on_select, props=dict(color='red', linewidth=2, markeredgecolor='red', markerfacecolor='red'))
    ...
    bounds = [
        (65.0, 99.0), # THRESHOLD_HI
        (55.0, 95.0), # THRESHOLD_LO
        (2.0, 30.0),  # BG_SIGMA 
        (0.01, 0.15), # CLAHE_CLIP 
        (5.0, 60.0),  # MIN_SKEL_LEN_PX (Expanded to allow 10um objects at 0.75um/px)
        (5.0, 50.0),  # MAX_WIDTH_PX (Expanded)
        (1.0, 5.0),   # MIN_LENGTH_WIDTH_RATIO 
        (3.0, 999.0), # MAX_TORTUOSITY
        (10.0, 999.0),# MAX_BRANCH_NODES
        (5.0, 999.0)  # MAX_ENDPOINT_COUNT
    ]
    ...
    with open("tuning_results_saturn.json", "w") as f:
        json.dump(results_list, f, indent=2)
    print("Full search history saved to 'tuning_results_saturn.json'.")
