#!/usr/bin/env python3
"""
Refined 3D biology-aware tuner v2 for the Saturn spermatid pipeline.

Focus of this version
---------------------
This version is tuned specifically to improve:
1. taper outliers
2. single-slice track burden

It keeps the same core 3D biological targets as the prior refined tuner:
- median 3D length near ~10 µm
- median tortuosity near ~1.1
- median thickness near ~1.2 µm

But it adds:
- stronger penalties for taper outlier fraction
- stronger penalties for single-slice fraction
- stronger reward for multi-slice fraction
- optional direct penalty on very short tracks that often feed single-slice artifacts

Recommended usage
-----------------
python tune_sperm_3d_objective_refined_v2.py --dir "C:\\Users\\dmishra\\Desktop\\sperm images" --slices 0-12 --new-roi
"""
...
try:
    from sperm_segmentation_saturnv2 import (
        CONFIG,
        segment_slice,
        measure_spermatids,
        track_across_slices,
    ...
    print("Error: Could not import sperm_segmentation_saturn.py from the current directory.")
    raise

ROI_SAVE_PATH = "last_drawn_roi_saturnv2.tif"
...
TARGETS = {
    ...
    "TAPER_GOOD_MAX": 1.60,
    "TAPER_HARD_MAX": 2.20,
    "MAX_EXTREME_TAPER_FRAC": 0.020,

    # Track structure
    "MAX_RAW_TO_3D_RATIO": 3.8,
    "TARGET_MULTI_SLICE_FRACTION": 0.56,
    "MIN_MULTI_SLICE_FRACTION": 0.50,
    "MAX_SINGLE_SLICE_FRACTION": 0.45,

    # Long-tail behavior
    "MAX_LONG_TRACK_FRAC": 0.040,
    "MAX_EXTREME_TORT_FRAC": 0.015,
    "MAX_SHORT_TRACK_FRAC": 0.10,   # tracks shorter than lower target range
}
...
def get_track_columns(df_tracks):
    ...
    length_col = next((c for c in ["length_3d_um_est", "total_3d_length_um", "max_length_um"] if c in cols), None)
    tort_col = next((c for c in ["tortuosity_3d", "tortuosity"] if c in cols), None)
    thick_col = next((c for c in ["thickness_um", "effective_thickness_um", "median_width_um"] if c in cols), None)
    taper_col = next((c for c in ["taper_ratio", "morphological_taper_ratio"] if c in cols), None)
    nslices_col = next((c for c in ["n_slices", "n_detections"] if c in cols), None)
...
def score_tracks(df_2d, df_tracks):
    ...
    taper_med = safe_median(taper, default=1.0)
    ...
    frac_short = safe_fraction(l3d < TARGETS["L3D_TARGET_RANGE_UM"][0], default=0.0)
    ...
    # Reward plausible populations
    score += 420.0 * frac_len_good
    score += 220.0 * frac_len_hard
    score += 220.0 * frac_tort_good
    score += 140.0 * frac_tort_hard
    score += 180.0 * frac_thick_good
    score += 100.0 * frac_thick_hard
    score += 260.0 * frac_taper_good
    score += 160.0 * frac_taper_hard

    # Median alignment
    score -= 110.0 * abs(l3d_med - TARGETS["L3D_MEDIAN_UM"])
    if np.isfinite(tort_med):
        score -= 120.0 * abs(tort_med - TARGETS["TORT_MEDIAN"])
    if np.isfinite(thick_med):
        score -= 90.0 * abs(thick_med - TARGETS["THICK_MEDIAN_UM"])
    if np.isfinite(taper_med):
        score -= 90.0 * abs(taper_med - TARGETS["TAPER_MEDIAN"])

    # Over-fragmentation
    if raw_to_3d > TARGETS["MAX_RAW_TO_3D_RATIO"]:
        score -= 220.0 * (raw_to_3d - TARGETS["MAX_RAW_TO_3D_RATIO"])

    # Stronger single-slice / multi-slice handling
    if np.isfinite(single_slice_frac) and single_slice_frac > TARGETS["MAX_SINGLE_SLICE_FRACTION"]:
        score -= 550.0 * (single_slice_frac - TARGETS["MAX_SINGLE_SLICE_FRACTION"])
    if np.isfinite(multi_slice_frac) and multi_slice_frac < TARGETS["MIN_MULTI_SLICE_FRACTION"]:
        score -= 420.0 * (TARGETS["MIN_MULTI_SLICE_FRACTION"] - multi_slice_frac)
    if np.isfinite(multi_slice_frac):
        score -= 180.0 * abs(multi_slice_frac - TARGETS["TARGET_MULTI_SLICE_FRACTION"])

    # Long-tail penalties
    if frac_long > TARGETS["MAX_LONG_TRACK_FRAC"]:
        score -= 520.0 * (frac_long - TARGETS["MAX_LONG_TRACK_FRAC"])
    if frac_extreme_tort > TARGETS["MAX_EXTREME_TORT_FRAC"]:
        score -= 700.0 * (frac_extreme_tort - TARGETS["MAX_EXTREME_TORT_FRAC"])
    if frac_extreme_taper > TARGETS["MAX_EXTREME_TAPER_FRAC"]:
        score -= 900.0 * (frac_extreme_taper - TARGETS["MAX_EXTREME_TAPER_FRAC"])
    if frac_short > TARGETS["MAX_SHORT_TRACK_FRAC"]:
        score -= 320.0 * (frac_short - TARGETS["MAX_SHORT_TRACK_FRAC"])

    if np.isfinite(l3d_iqr) and l3d_iqr > 4.2:
        score -= 90.0 * (l3d_iqr - 4.2)
...
def objective_fn(x, um_per_px):
    ...
    if score > best_global_score:
        ...
        msg = (
            ...
            f" | taper med={metrics.get('taper_median', 'NA')}"
...
def main():
    ...
    if base_dir == r"C:\Users\dmishra\Desktop\sperm images":
        root = tk.Tk()
        root.withdraw()
        ...
        slice_str = simpledialog.askstring("Z-Slice Range", "Enter consecutive slices (e.g. 0-12):", initialvalue="0-12")
        root.destroy()
        if not slice_str:
    ...
    print(f"\nLoaded {len(images_to_eval)} images for refined 3D optimization v2: {z_values_eval}")
    roi_mask_global = build_roi(images_to_eval, force_redraw=args.new_roi)

    bounds = [
        (76.0, 96.0),   # THRESHOLD_HI
        (62.0, 88.0),   # THRESHOLD_LO
        (5.0, 24.0),    # BG_SIGMA
        (0.02, 0.10),   # CLAHE_CLIP
        (6.0, 18.0),    # MIN_SKEL_LEN_PX
        (4.0, 11.0),    # MAX_WIDTH_PX
        (1.8, 6.0),     # MIN_LENGTH_WIDTH_RATIO
        (1.2, 6.0),     # MAX_TORTUOSITY
        (0.0, 10.0),    # MAX_BRANCH_NODES
        (2.0, 10.0),    # MAX_ENDPOINT_COUNT
    ]

    print("\n--- Starting refined 3D biology-aware optimization v2 ---")
    print("Extra emphasis in this version:")
    print("  - taper cleanup")
    print("  - single-slice cleanup")
    print("  - keep 3D median length near ~10 µm")
    ...
    for k in [
        ...
        "frac_len_good", "frac_long", "frac_short", "frac_extreme_tort", "frac_extreme_taper"
    ]:
        print(f"{k:24s}: {best.get(k)}")
    print(f"Parameters: {best['params']}")

    out_json = "tuning_results_saturnv2.json"
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(results_list, f, indent=2)
    print(f"Full search history saved to {out_json}")
