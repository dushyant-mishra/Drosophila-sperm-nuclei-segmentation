...
_VERSION = "v3_recovered_baseline"
...
def generate_excel_report(out_dir, df, df_summary, df_tracks=None):
    ...
    try:
        with pd.ExcelWriter(excel_path, engine='xlsxwriter') as writer:
            ...
            # --- Sheet 2: 3D Morphometrics (Quality) ---
            if df_tracks is not None and not df_tracks.empty:
...
def generate_batch_report(out_dir, df, df_summary, um, df_tracks=None, gui_callback=None):
    ...
    try:
        with PdfPages(pdf_path) as pdf:
            ...
            # --- PAGE 2: 3D MORPHOMETRICS SUMMARY (QUALITY-FILTERED) ---
            if df_tracks is not None and not df_tracks.empty:
                ...
                # Save 3d_length_distribution.png for Excel embedding (quality-only)
...
def generate_pptx_report(out_dir, df, df_summary, um, df_tracks=None):
    ...
    try:
        ...
        # ---------------------------------------------------------------------
        # SLIDE 3: Advanced 3D Biometrics (Quality-Filtered)
        # ---------------------------------------------------------------------
        if df_tracks is not None and not df_tracks.empty:
            ...
            # For Slide 3, we show the histograms of the QUALITY population
